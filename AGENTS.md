...
- Five hand-authored top-level pages, each a full standalone HTML document with duplicated
  header/nav and footer markup: `index.html` (home), `services.html`, `a-propos.html` (about),
  `articles.html` (article listing), `contact.html`. There is no templating — shared markup
  (header, nav, footer) must be edited in all these files when changed, plus every page in
  `articles/`.
- `articles/` holds one standalone HTML page per article (relative links use `../`).
